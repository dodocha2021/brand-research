...
export async function POST(req: NextRequest) {
  ...
  try {
    // 启动异步任务
    console.log(`Sending TikTok task to Apify with URL: ${url}, name: ${name}`);
    ...
    if (!res.ok) {
      ...
      console.error(`TikTok API call failed with status ${res.status}:`, errorText);
    ...
    console.log(`TikTok webhook task response:`, JSON.stringify(data, null, 2));
    
    if (!data.id) {
      console.error("TikTok task started but returned no ID:", data);
    }
