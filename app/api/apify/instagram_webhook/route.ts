...
export async function POST(req: NextRequest) {
  ...
  // Instagram任务输入格式
  const taskInput = {
    usernames: [url], // 用户名或URL
    competitorName: name, // 保留competitor名称用于数据库
    searchId: searchId // 保留searchId用于webhook回调
  }
  
  try {
    // 启动异步任务
    console.log(`Sending Instagram task to Apify with URL: ${url}, name: ${name}`);
    console.log(`Task payload:`, JSON.stringify(taskInput, null, 2));
    
    const res = await fetch(
      `https://api.apify.com/v2/actor-tasks/ai.labs~instagram-profile-scraper-branddeepresearch/runs?token=${apiKey}`,
      {
        method: 'POST',
    ...
    if (!res.ok) {
      ...
      console.error(`Instagram API call failed with status ${res.status}:`, errorText);
    ...
    console.log(`Instagram webhook task response:`, JSON.stringify(data, null, 2));
    
    if (!data.id) {
      console.error("Instagram task started but returned no ID:", data);
    }
    ...
    console.error("Instagram任务启动失败:", e);
    return NextResponse.json({ 
      error: e.message, 
      stack: e.stack,
