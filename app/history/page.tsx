...
export default function HistoryPage() {
  ...
  return (
    <div className="container">
      ...
      {loading ? (
        <div style={{ textAlign: 'center', color: '#2563eb', padding: 48 }}>Loading...</div>
      ) : brandList.length === 0 ? (
        <div style={{ textAlign: 'center', color: '#888', padding: 48 }}>No search history</div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          {pagedBrands.map((brand) => (
            <div key={brand} style={{ background: '#fff', borderRadius: 16, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', padding: 24 }}>
              <div
                style={{ display: 'flex', flexWrap: 'wrap', gap: 16, alignItems: 'center', cursor: 'pointer' }}
                onClick={() => setExpanded(expanded === brand ? null : brand)}
              >
                <span style={{ fontWeight: 600, fontSize: 18 }}>Brand: {brand}</span>
                <span style={{ marginLeft: 'auto', color: '#2563eb', textDecoration: 'underline', fontWeight: 500 }}>
                  ...
                  <table className="result-table">
                    ...
                    <tbody>
                      {grouped[brand] ? (
                        ...
                        <tr>
                          <td colSpan={grouped[brand] ? Object.keys(grouped[brand][0] || {}).length - 2 : 1} style={{ textAlign: 'center', color: '#888', padding: 24 }}>No competitors</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
